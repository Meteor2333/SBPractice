...
public class Spectator extends SubCommand {
    private static ArrayList<UUID> specList = new ArrayList<>();

    public Spectator(MainCommand parent, String name) {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (specList.contains(player.getUniqueId())) {
            for (Plot plot : Plot.getPlots()) {
                if (plot.getPlotStatus().equals(PlotStatus.NOT_OCCUPIED)) {
                    ...
                    player.sendMessage(Message.getMessage("spec-mode-disabled"));
                    specList.remove(player.getUniqueId());
                    return;
                }
            } player.sendMessage(Message.getMessage("plot-full"));
        } else {
            specList.add(player.getUniqueId());
            ...
            player.setAllowFlight(true);
            player.sendMessage(Message.getMessage("spec-mode-enabled"));
        }
    }

    public static boolean check(Player player) {
        return specList.contains(player.getUniqueId());
    }

    public static void removeFromSpecList(UUID key) {
        specList.remove(key);
    }
}
