...
public class v1_8_R3 extends NMS {
    ...
    @Override
    public @Nullable String getItemTag(ItemStack itemStack, String key) {
    ...
    public void hidePlayer(Player player) {
        ...
        try {
            ...
            gm.set(pim, WorldSettings.EnumGamemode.SPECTATOR);
            ...
            gm.set(pim, WorldSettings.EnumGamemode.CREATIVE);
    ...
    public void showPlayer(Player player) {
        ...
        player.getWorld().getPlayers().forEach(p -> p.showPlayer(player));
        ...
        switch (state.getType()) {
            ...
            case SKULL:
                if (state instanceof CraftSkull) {
                    ...
                    result.append(":").append(skull.getRotation().name());
                    result.append(":").append(skull.getSkullType().name());
                }
                break;
            case WOOD_STAIRS:
    ...
    public boolean isSimilarBlockState(BlockState state1, BlockState state2) {
        if (!state1.getData().equals(state2.getData())) {
            //Bukkit.getConsoleSender().sendMessage(state1.getData().getClass().getName() + " " + state2.getData().getClass().getName());
    ...
    public CompoundTag getTileEntityData(Block block) {
        ...
        if (entity instanceof TileEntityBanner) {
            ...
            tag.putInt("Rotation", skull.getRotation());
        }
        return tag;
    }
